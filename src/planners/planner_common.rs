...
pub fn get_neighbors_4_con(pos: (u32, u32), gridmap: &Gridmap) -> Vec<(u32, u32)> {
  ...
  if pos_0_non_zero {
    if gridmap.xy_in_freespace((pos.0 - 1, pos.1)){
  ...
  if pos_1_non_zero {
    if gridmap.xy_in_freespace((pos.0, pos.1 - 1)){
...
pub fn get_neighbors_8_con(pos: (u32, u32), gridmap: &Gridmap) -> Vec<(u32, u32)> {
  ...
  if pos_0_non_zero {
    ...
    if gridmap.xy_in_freespace(left){
      neighbors.push(left); // Left (x-1, y))
    }

    if pos_1_non_zero {
      let top_left = (pos.0 - 1, pos.1 - 1);
      if gridmap.xy_in_freespace(top_left){
  ...
  if pos_0_non_max {
    ...
    if gridmap.xy_in_freespace(right){
      neighbors.push(right); // Right (x+1, y)
    }
    
    if pos_1_non_max {
      let bottom_right = (pos.0 + 1, pos.1 + 1);
      if gridmap.xy_in_freespace(bottom_right){
  ...
  if pos_1_non_zero{
    ...
    if gridmap.xy_in_freespace(top){
      neighbors.push(top); // Top (x, y-1)
    }

    if pos_0_non_max {
      let top_right = (pos.0 + 1, pos.1 - 1);
      if gridmap.xy_in_freespace(top_right){
  ...
  if pos_1_non_max {
    ...
    if gridmap.xy_in_freespace(bottom){
      neighbors.push(bottom); // Bottom (x, y+1), (idx)
    }

    if pos_0_non_zero {
      let bottom_left = (pos.0 - 1, pos.1 + 1);
      if gridmap.xy_in_freespace(bottom_left){
...
pub fn trace_path(goal_cell: &(u32, u32), parents: &HashMap<(u32, u32), (u32, u32)>) -> Vec<(u32, u32)>{
  let mut path: Vec<(u32, u32)> = Vec::new();
  let mut cur_cell = goal_cell;
  path.push(*cur_cell);

  while parents.get(&cur_cell).unwrap() != cur_cell {
    // println!("compare: {:?} vs {:?}", parents.get(&cur_cell).unwrap(), cur_cell);
